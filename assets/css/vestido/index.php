   ...
   <main>
                     ...
                     <div class="row align-items-center">
                        <div class="col-xl-6 col-lg-6 col-md-6">
                           <div class="tp-slider-content-2">
                              ...
                              <h3 class="tp-slider-title-2">Discover, Shop, Repeat.</h3>
                     ...
                     <div class="row align-items-center">
                        <div class="col-xl-6 col-lg-6 col-md-6">
                           <div class="tp-slider-content-2">
                              ...
                              <h3 class="tp-slider-title-2">Fragrance Finds for Every Mood.</h3>
                              <!-- <div class="tp-slider-btn-2">
                                 <a href="shop.html" class="tp-btn tp-btn-border">Shop Collection</a>
                              </div> -->
                           </div>
      ...
      <style>
         .tp-category-main-thumb {
            /* Set a maximum width to constrain the image size */
         ...
         .include-bg {
            /* Ensure the image takes up 100% of the container's width */
            width: 100%;
            object-fit: cover;
         ...
         <div class="container">
               ...
               <div class="col-xl-12">
                  <div class="tp-section-title-wrapper-2 text-center mb-50">
                     ...
                     <div class="tp-history-year" >
                                             <p style="font-size:25px;color:#000000;line-height: 1;">CATEGORIES ON THE VESTIDO STORE.</p>
                                          </div>
                     <h3 ></h3>
                  </div>
               </div>
            ...
            <div class="row">
               ...
               while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  // Query to get the total count of products for this category
                  $pro = $admin->ret("SELECT COUNT(*) AS total_products FROM `product` WHERE `c_id` = $categoryId");
                  $prodata = $pro->fetch(PDO::FETCH_ASSOC);
                  $totalProducts = $prodata['total_products']; {
               ?>
                     <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6">
                        <div class="tp-category-main-box mb-25 p-relative fix" data-bg-color="#F3F5F7"><img class="tp-category-main-thumb include-bg transition-3" src="controller/<?= $row['c_image']; ?>" alt="Category Image">

                           <div class="tp-category-main-content" style="background-color: #fdfdfdd6">
                              <h3 class="tp-category-main-title">
                                 <a href="products.php?category=<?= $row['c_id']; ?>"><?= $row['c_name']; ?></a>
                              </h3>
         ...
         <div class="container">
            <div class="row">
               <div class="col-xl-12">
                  ...
                  <div class="tp-history-year" >
                                             <p style="font-size:25px;color:#000000;line-height: 1">CUSTOMER'S FAVOURITE STYLE PRODUCT</p>
                                          </div>
                     <h3 class="tp-section-title-2"></h3>
                  </div>
               </div>
            </div>
            <div class="row">
                        ...
                        <div class="nav nav-tabs justify-content-center" id="nav-tab" role="tablist">
                           <button class="nav-link active" id="nav-allCollection-tab" data-bs-toggle="tab" data-bs-target="#nav-allCollection" type="button" role="tab" aria-controls="nav-allCollection" aria-selected="true">
                              ...
                              <span class="tp-product-tab-tooltip">
                                 <?php
                                 // Query to get the total count of all products
                                 $allProductsQuery = $admin->ret("SELECT COUNT(*) AS total_products FROM product where  `p_status` = 'Active'");
                                 $allProductsData = $allProductsQuery->fetch(PDO::FETCH_ASSOC);
                                 $totalAllProducts = $allProductsData['total_products'];
                                 echo $totalAllProducts;
                                 ?>
                              </span>
                           </button>
                           <?php
                           $cats = $admin->ret("SELECT * FROM `category`");
                           while ($catsdata = $cats->fetch(PDO::FETCH_ASSOC)) {
                              $categoryId = $catsdata['c_id'];
                              $categoryName = $catsdata['c_name'];

                              // Query to get the total count of products for this category
                              $pro = $admin->ret("SELECT COUNT(*) AS total_products FROM `product` WHERE `c_id` = $categoryId and `p_status` = 'Active' ");
                              $prodata = $pro->fetch(PDO::FETCH_ASSOC);
                              $totalProducts = $prodata['total_products'];
                           ?>
                              <button class="nav-link" id="nav-<?= $categoryId ?>" data-bs-toggle="tab" data-bs-target="#nav-<?= $categoryId ?>" type="button" role="tab" aria-controls="nav-<?= $categoryId ?>" aria-selected="false">
                                 <?= $categoryName ?>
                                 <span class="tp-product-tab-tooltip"><?= $totalProducts ?></span>
                              </button>
                           <?php } ?>
                        </div>
                     </nav>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchProducts('allCollection', 'All Collection');

    const categoryButtons = document.querySelectorAll('.nav-link');

    categoryButtons.forEach(button => {
        button.addEventListener('click', function() {
            const categoryId = button.getAttribute('data-bs-target').replace('#nav-', '');
        ...
        fetch('get_products.php?category_id=' + categoryId)
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Loop through products and populate the container
                    products.forEach(product => {
                        const productItem = `
                            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6">
                            <div class="tp-product-item-2 mb-40">
                            <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
                                <a href="productpage.php?product=${product.p_id}">
                                    <img src="controller/${product.p_image1}" alt="${product.p_name}">
                                </a>
                                <!-- product action -->
                                <div class="tp-product-action-2 tp-product-action-blackStyle">
                                    <div class="tp-product-action-item-2 d-flex flex-column">
                            ...
                            <div class="tp-product-content-2 pt-15">
                                ...
                                <h3 class="tp-product-title-2">
                                    <a href="productpage.php?product=${product.p_id}">${product.p_name}</a>
                                </h3>
